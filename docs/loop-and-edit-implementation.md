...
3. Tool result text passes to the model unclipped: the full body read from the tool log (when the session has one) or the inline event text (legacy). No cap, no marker. The log split (slim index in `events.jsonl`, full body in `tools.jsonl`) keeps the event log light for the TUI; the model input carries the full text.

...
| `tui-file-picker.md`                           | Spec, not yet built | 2026-09-08   | Spec for the `@` file picker and the reusable completion window widget: fuzzy from day 0 via `frizbee`, the `picker/` middle layer (items, matcher, state, render, preview), the `@` trigger, the settled floating display with a file content preview pane, and the phased build plan |

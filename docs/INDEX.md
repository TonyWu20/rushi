...
| `reference/nix/nix-flake-module.md` | Active              | 2026-09-09   | The Nix flaking module design: `lib.mkRushi`, the `rushi.*` option schema, `lib.fetchExt`/`fetchTool` helpers, three version-pinning modes, NixOS/home-manager modules, pi-flake capability mapping. Bundled into the binary (`rushi docs`) with the rest of `docs/reference/`. Companion: `reference/nix/ext-flake-authoring.md` (producer side) |
| `reference/nix/ext-flake-authoring.md` | Active              | 2026-09-12   | How an extension repo writes its `flake.nix`: the four `$out` contracts (`external_tools` / `external_ui_extensions` / `external_hooks` / `tui`), the `buildCrate` + tool/ext wrapper pattern, any-combination of tool+hook+ext in one flake, Mode A (no flake, `lib.fetchTool`) vs Mode B (ext flake) decision, Properties P1-P4. Bundled into the binary (`rushi docs`) with the rest of `docs/reference/` |

...
| `phase-2-plan.md`                             | Spec, not yet built | 2026-09-03   | The Phase 2 spec and plan: the `harness` loop binary (`run`/`step`, the session lock, the cancellation handles, the ported retry and compact-branch semantics), the `harness-common` utility crate (one `LogLine`, one validator, one compact math), five stages with gates |
...
| `loop-lifecycle-hooks.md`                        | Spec, not yet built | 2026-09-08   | The lifecycle-window hook design: fine-grained control points on the loop (`session.start/end`, `step.start/end`, `model.before/after`, `compact.before/after`, `overflow.resolve`, `exhausted.handle`, `tool.before/after`), a Unix-style hook ABI (command on a path, JSON stdin/stdout, exit-code decision), the two built-in overflow strategies as swappable hook registrations, and the Unix-philosophy reframe of "hooks" as applications on a path |

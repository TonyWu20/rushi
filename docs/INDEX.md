...
| `config-dump.md`                                  | Implemented         | 2026-09-18   | The `rushi config` subcommand: dumps the resolved active config file (resolution order `$CONFIG`, `--config`, Nix side-by-side, CWD) to stdout verbatim with a trailing newline, so it can be redirected to disk, tweaked, and fed back via `--config` |

...
| `refinement-policy.md`                        | Active              | 2026-09-15   | Rules for changing the harness: evidence bar, trigger thresholds, not-yet list. G2a crash consistency is done via `scripts/crash-e2e.sh`. G2b is parked in `docs/itches.md`. P1b records that the TUI may extend the event vocabulary for rendering |
...
| `itches.md`                                     | Active              | 2026-09-12   | Parked-itch log: open observations and promotion candidates (shared-crate extraction, validator unification, DRT ops). Open entry: G2b (tool side effects repeat after a mid-tool crash) |

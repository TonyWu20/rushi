...
| `reference/nix/nix-flake-module.md` | Active              | 2026-09-18   | The Nix flaking module design: `lib.mkRushi`, the `rushi.*` option schema, `lib.fetchExt`/`fetchTool` helpers, three version-pinning modes, NixOS/home-manager modules, pi-flake capability mapping. Since issue #13, `meta.rushi` producer declarations drive eval-time filling of `extension_tool_paths` / `[ui_extensions]` (see `meta-rushi.md`). Bundled into the binary (`rushi docs`) with the rest of `docs/reference/`. Companion: `reference/nix/ext-flake-authoring.md` (producer side) |

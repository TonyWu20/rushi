...
| `failure-tracking.md`                           | Active              | 2026-09-18   | Known failures and fixes, one entry per defect (FT-001 through FT-027). FT-026 (an unknown tool name aborts the loop) is fixed by correction 65. Parse forwards unknown or blank names and route reports the not-run result. FT-027 (parse hard-fail on malformed arguments) remains open and deferred |

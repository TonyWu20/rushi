...
| `refinement-policy.md`                        | Active              | 2026-09-16   | Rules for changing the harness: evidence bar, trigger thresholds, not-yet list. G2a crash consistency is done via `scripts/crash-e2e.sh`. G2b is parked in `docs/itches.md`. P1b records that the TUI may extend the event vocabulary for rendering. P8 cross-references the Phase-3 re-evaluation gate |
...
| `phase-2-crate-research.md`                   | Investigation     | 2026-09-15   | Crate research for the Phase 2 core: maps the shell glue of `step.sh`/`turn.sh` to Rust crates (`grep` family, `jaq`, `jsonschema`, `toml`, `regex`, `chrono`). Core rows landed (`toml`, `regex`, `serde`, `chrono`). All non-core rows retired 2026-09-15: the four native tools suffice, and the typed `Event` enum is the validator. See its Decisions section |
...
| `loop-lifecycle-hooks.md`                        | Implemented | 2026-09-24   | The lifecycle-window hook design: fine-grained control points on the loop (`session.start/end`, `step.start/end`, `model.before/after`, `compact.before/after`, `overflow.resolve`, `exhausted.handle`, `tool.before/after`, `run.idle`), a Unix-style hook ABI (command on a path, JSON stdin/stdout, typed exit codes 0/2/3 for ok/abort/fail, state-field effects replacing the retired decision vocabs), per-window pipelines via `[hooks.defs.<name>]` + `[hooks.pipeline."<window>"]` with chain markers (issue #38 hard cutover, `[[hooks.on]]` retired; includes the `goal-tokens` mis-registration fix), and the Unix-philosophy reframe of "hooks" as applications on a path. Built: `crates/rushi/src/hooks.rs` (pipeline runner), `bin/hook-compact` |
...
| `reference/nix/nix-flake-module.md` | Active              | 2026-09-24   | The Nix flaking module design: `lib.mkRushi`, the `rushi.*` option schema, `lib.fetchExt`/`fetchTool` helpers, three version-pinning modes, NixOS/home-manager modules, pi-flake capability mapping. Since issue #13, `meta.rushi` producer declarations drive eval-time filling of `extension_tool_paths` / `[ui_extensions]` (see `meta-rushi.md`). Issue #38 cutover: hook declarations are `hooks.defs.<name>` + `hooks.pipeline."<window>"`; the legacy `on` default is dropped from the Nix side (decision 2026-09-24, recorded in the doc's migration section). Bundled into the binary (`rushi docs`) with the rest of `docs/reference/`. Companion: `reference/nix/ext-flake-authoring.md` (producer side) |
| `reference/nix/ext-flake-authoring.md` | Active              | 2026-09-12   | How an extension repo writes its `flake.nix`: the four `$out` contracts (`external_tools` / `external_ui_extensions` / `external_hooks` / `tui`), the `buildCrate` + tool/ext wrapper pattern, any-combination of tool+hook+ext in one flake, Mode A (no flake, `lib.fetchTool`) vs Mode B (ext flake) decision, Properties P1-P5 (P5: `meta.rushi` producer declarations, issue #13). Bundled into the binary (`rushi docs`) with the rest of `docs/reference/` |
| `meta-rushi.md`                              | Implemented     | 2026-09-18   | Issue #13: eval-time `meta.rushi` producer declarations replace #10 build-time glob discovery for `extension_tool_paths` and `[ui_extensions] enabled`. Consumer-set values remain authoritative. Legacy/plain-path sources fall back to build-time discovery with a warning. Properties P1-P8, verified by `bash tests/nix/run.sh` |
...
| `failure-tracking.md`                           | Active              | 2026-09-21   | Known failures and fixes, one entry per defect (FT-001 through FT-028). FT-026 (an unknown tool name aborts the loop) is fixed by correction 65. Parse forwards unknown or blank names and route reports the not-run result. FT-027 (parse hard-fail on malformed arguments) is fixed by correction 66. FT-028 (the PR #26 regression test failed the macOS CI) is fixed by PR #28 |
...
| `reasoning-log-size.md`                           | Implemented         | 2026-09-18   | Log-size decision: strip the redundant reasoning `summary` field at the `bin/parse` step (measured 99.9% byte overlap with `content`). Forward-only; `content` stays whole. No capping or trimming introduced. Records the deliberate deviation from the verbatim-reasoning contract in `typed-events.md` |
| `config-dump.md`                                  | Implemented         | 2026-09-18   | The `rushi config` subcommand: dumps the resolved active config file (resolution order `--config`, `$CONFIG`, Nix side-by-side, CWD; issue #36 made the flag outrank the env var) to stdout verbatim with a trailing newline, so it can be redirected to disk, tweaked, and fed back via `--config` |
| `itches.md`                                     | Active              | 2026-09-16   | Parked-itch log: open observations and promotion candidates (shared-crate extraction, validator unification, DRT ops). Open entries: G2b (tool side effects repeat after a mid-tool crash) and core-crate extraction parking (2026-09-16) |
...
| `image-read-kiss.md`                             | Implemented         | 2026-09-10   | KISS decision: image reading as a kernel + extension split. Kernel: accept image tool results, the per-model `vision` flag, the Responses `input_image` / Chat-Completions `image_url` wire forms, the flat per-image token estimate, the `tool.after` `results` state field, and the `hook_io` state builder for extensions. Extension: image detection/compression/re-encode. e2e in `scripts/tool-after-transform-e2e.sh` |
| `kernel-complexity-audit.md`                    | Review            | 2026-09-16   | Redundancy and complexity audit of the kernel against `architecture.md`, `skill-remapped-to-os-apps.md`, and `refinement-policy.md`. Maps the subagent-design kernel deltas (D6/D8 landed, D1/D3/D5/D7 unbuilt). Findings F1-F8 with the sequenced follow-up. Gate clean |

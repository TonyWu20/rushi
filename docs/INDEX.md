...
| `pi-extension-port-investigation.md`            | Investigation       | 2026-09-08   | Portability assessment of the `pi-config` extension set (pi-goal, rpiv-ask-user-question, pi-fff, agent-simple-english, no-find-grep, no-bare-python, pi-lynx, pi-terminal-browser, pi-automode) onto the harness: tool/hook/UI/loop mapping, difficulty verdict, blockers, and suggested order. pi-goal verdict updated to "Ready" (2026-09-08). |
| `pi-goal-readiness.md`                           | Investigation     | 2026-09-08   | Readiness assessment for porting `pi-goal` as a harness extension: all three loop seams (`run.idle`, `compact.before`, `model.before` transform) are built and gated; the remaining work is application-level (goal tools, hook binaries, state file); gaps G2-G4 with workarounds |
| `goal-rule-reinject.md`                           | Superseded by `system-prompt-generation.md` | 2026-09-08   | Supersedes the idle-rules-only proposal: inject the goal block into `request.instructions` via a byte-stable `model.before` transform. Superseded by `system-prompt-generation.md` (kernel-join model with `prompt_fragments`). D1-D3 and cache tradeoff carried forward. |
...
| `aeneas-rust-to-lean.md`                       | Investigation       | 2026-09-07   | Aeneas Rust-to-Lean toolchain: charon MIR extraction + aeneas translation, the functional-core/imperative-shell pattern shared with this harness |
...
| `failure-tracking.md`                           | Active              | 2026-09-06   | How to track and report harness failures: the `error` event, `hook.<window>.error` markers, and the fenced-failure report format |
| `goal-ui_feedback_from_human.md`                | Historical          | 2026-09-07   | Feedback notes on the goal UX redesign (pre-split; goal-ux.md now lives in `rushi-tui/docs/`) |
...
| `vim-editor-design.md`                           | Implemented         | 2026-09-05   | Design for the TUI's built-in vim-like editor: keymap, mode machine, register model. TUI code lives in `rushi-tui` |

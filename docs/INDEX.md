...
| `handoff-strategy.md`                           | Spec, not yet built | 2026-09-08   | The handoff context strategy: replaces the in-session last-resort compact with a summary handoff that seeds and auto-starts a new session. The `ContextStrategy` + `SessionStore` port, the handoff doc format (both session dirs), and the log-index convention. The `compact_strategy` config knob selects handoff or compact |

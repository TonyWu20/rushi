...
config-driven `extension_tool_paths` mechanism.
...
It is the union of the `native_tool_paths` and `extension_tool_paths`
...
It reads `native_tool_paths` and `extension_tool_paths` from the config.
...
Order: `native_tool_paths` first, then `extension_tool_paths`, each in config order.
Then each `extension_tool_paths` entry in config order,
...
- add a comment: `# The tool list is generated by assemble from native_tool_paths + extension_tool_paths.`
...
P1. generated-tool-list: given `native_tool_paths` with tool manifests
    and optional `extension_tool_paths`, observe that `assemble`

...
catalog is `tools --list` (TUI and human inspection). The agent's tool
list is in the prompt prefix, generated by `assemble` from the
manifests. The prefix is byte-stable in steady state.
A tool-set change costs one prefix rebuild.
A fenced failure reports. It does not hang.
Publishing adds the distribution
layer only.
...
self-doc (P2 there), catalog (P3 there), prefix-stability (P4 there),

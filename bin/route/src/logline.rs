...
//! See `notes/itches.md`). Keep the copies in sync. This copy serves
//! the per-session tool log (docs/tool-log-design_from_human.md).
...
mod tests {
    ...
    fn commit_appends_one_complete_line_per_event() {
        ...
        LogLine::from_json(r#"{"v":1,"type":"tool_result","ts":"t","content":"one"}"#)
            .commit(&log)
            .unwrap();
        LogLine::from_json(r#"{"v":1,"type":"tool_result","ts":"t","content":"two"}"#)
            .commit(&log)
            .unwrap();

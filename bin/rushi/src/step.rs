...
fn fire_step_start(
    cfg: &HarnessConfig,
    session: &SessionDir,
    claim: &rushi_common::stage::Claim,
) {
...
fn try_compact_with_hooks(
    ...
    force: bool,
    strip_last: bool,
) -> rushi_common::stage::CompactStatus {
    let payload = serde_json::json!({
...
fn post_compact_sanity(
    cfg: &HarnessConfig,
    session: &SessionDir,
    status: &rushi_common::stage::CompactStatus,
) {
    if status.outcome != CompactOutcome::Compacted {
        return;
    }
...
fn run_awaiting_approval(
    ...
    runner: &SubprocessRunner,
    session: &SessionDir,
    _claim: &rushi_common::stage::Claim,
    mode: StepMode,
) {
    let path = session.path.join("events.jsonl");
    let Ok(data) = std::fs::read_to_string(&path) else {
...
fn route_batch(
    ...
    calls: &[ToolCallEvent],
    session: &SessionDir,
) -> Vec<rushi_common::stage::ToolResultEvent> {
    if calls.is_empty() {
        return Vec::new();
    }
    let cwd = {
